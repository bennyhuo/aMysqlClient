...
public class FinalDb {
	 ...
	 * @return
	 */
	public <T> List<T> findAll(Class<T> clazz, int limit, int offset){
		checkTableExist(clazz);
		return findAllBySql(clazz,
				SqlBuilder.getSelectSQL(clazz)  + " LIMIT "+ limit + " OFFSET " + offset );
	}
	...
	public <T> List<T> findAllByWhere(Class<T> clazz, String strWhere,
		...
		checkTableExist(clazz);
		return findAllBySql(clazz,
				SqlBuilder.getSelectSQLByWhere(clazz, strWhere) + " LIMIT "+ limit + " OFFSET " + offset );
	}
	
	public <T> int countByWhere(Class<T> clazz, String strWhere){
		checkTableExist(clazz);
		return count(clazz,
				SqlBuilder.getCountSQLByWhere(clazz, strWhere));
	}
	
	public <T> int countAll(Class<T> clazz){
		checkTableExist(clazz);
		return count(clazz,
				SqlBuilder.getCountSQLByWhere(clazz, null));
	}
	...
	private <T> int count(Class<T> clazz, String strSQL) {
		checkTableExist(clazz);
		debugSql(strSQL);
		Cursor cursor = db.rawQuery(strSQL, null);
		int count = 0 ;
